"""
MSD Lung (Task06) 2D-slice dataset.

Expects the directory layout produced by prepare_data.py:
    <data_dir>/
        metadata.json
        train/
            images/<case_id>_z<z>.npy   [3, H, W] uint8   (multi-slice [z-1,z,z+1])
            labels/<case_id>_z<z>.npy   [H, W]    uint8   (0=cancer, 255=bg)
        val/
            images/ …
            labels/ …
...
class MSDLungDataset(Dataset):
    ...
    def __init__(
        ...
        self.num_classes = meta["num_classes"]      # 1
        self.class_names = meta["class_names"]      # ["cancer"]
    ...
    def __getitem__(self, idx: int):
        ...
        seg_map = lbl.clone()   # keep background=255

        fg_classes = torch.unique(lbl)
        fg_classes = fg_classes[(fg_classes != self.IGNORE_IDX) & (fg_classes >= 0)]

        if len(fg_classes) == 0:
            # No cancer in this slice: empty tensors let every query predict no-object.
            masks  = torch.zeros(0, *self.img_size, dtype=torch.bool)
            labels = torch.zeros(0, dtype=torch.long)
        else:
            masks  = torch.stack([lbl == c for c in fg_classes])  # [N, H, W] bool
            labels = fg_classes                                    # [N]
...
def collate_fn(batch):
    """Custom collate: stack images, keep targets as a list of dicts."""
